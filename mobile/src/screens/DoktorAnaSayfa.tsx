...
export default function DoktorAnaSayfa({ navigation }: any) {
  ...
  return (
    <View style={styles.kapsayici}>
      <View style={styles.baslikBar}>
        <View>
          <Text style={styles.hosgeldin}>Doktor Paneli</Text>
          <Text style={styles.ad}>Dr. {kullanici?.ad} {kullanici?.soyad}</Text>
        </View>
          ...
          renderItem={({ item }) => (
            <View style={styles.kart}>
              <View style={styles.kartUst}>
                <Text style={styles.hastaAd}>
                  {item.HastaAd} {item.HastaSoyad}
                </Text>
              ...
              {item.Notlar ? (
                <Text style={styles.notYazi}>Not: {item.Notlar}</Text>
              ) : null}
            </View>
          )}
        />
...
const styles = StyleSheet.create({
  ...
  hosgeldin: { color: '#d1fae5', fontSize: 13 },
  ...
  hastaAd: { fontSize: 15, fontWeight: '600', color: '#111827' },
  ...
  notYazi: { color: '#6b7280', fontSize: 12, marginTop: 4 },
});
