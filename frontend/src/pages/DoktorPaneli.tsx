...
export default function DoktorPaneli() {
  ...
  const [aktifSekme, setAktifSekme] = useState<'liste' | 'takvim' | 'calisma'>('liste')
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-10">
        ...
        <div className="flex gap-2 mb-6">
          {(['liste', 'takvim', 'calisma'] as const).map(s => (
            <button key={s} onClick={() => setAktifSekme(s)}
              ...
              {s === 'liste' ? '📋 Liste' : s === 'takvim' ? '📆 Haftalık Takvim' : '🕐 Çalışma Saatleri'}
            </button>
          ))}
        </div>
